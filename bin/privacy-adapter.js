...
const DEFAULT_CAW_EVM_ADDRESS = "0x8bf7aee000ccd484c7343346cd7666f52fde9e13";
const ALLOWED_DEPOSITS = [0.01, 0.1, 1, 10, 100];
const DEFAULT_NOTE_PASSWORD = "local-dev-password-change-me";
const LOCAL_NOTE_SECRET_VERSION = 1;
...
function newRunId(command = "run") {
  return `${new Date().toISOString().replace(/[-:.]/g, "").replace("T", "-").slice(0, 16)}-${safeFileSegment(command)}-${crypto.randomBytes(4).toString("hex")}`;
}
...
function returnCawWarning(cfg) {
  return {
    code: "PUBLIC_RETURN_LINK",
    severity: "warning",
    message: "return:caw is treasury reconciliation, not a privacy enhancement. The final burner-to-CAW USDC sweep is public and can link the private withdrawal burner to the CAW address.",
    defaultRecommendation: "For sensitive workflows, prefer spend-down or hold-note unless the user explicitly wants funds reconciled to CAW.",
    destination: cfg.cawEvmAddress,
...
function validateNotePassword(raw, source) {
  const value = String(raw || "").trim();
  if (!value) {
    throw new Error(`${source} is empty.`);
  }
  if (value === DEFAULT_NOTE_PASSWORD) {
    throw new Error(`${source} must not be the public default '${DEFAULT_NOTE_PASSWORD}'.`);
  }
  if (value.length < 12) {
...
function hasNoteFiles(noteDir) {
  if (!fs.existsSync(noteDir)) return false;
  return fs.readdirSync(noteDir).some((name) => name.endsWith(".json") && name.includes("px402-note"));
}

function noteSecretVaultPath() {
  const requested = process.env.PX402_NOTE_SECRET_FILE || process.env.PX402_SECRET_FILE;
  return path.resolve(rootDir, requested || "data/secrets/px402-note-password.json");
}

function writeLocalNoteSecret(file, password, overwrite = false) {
  if (fs.existsSync(file) && !overwrite) {
    const stored = JSON.parse(fs.readFileSync(file, "utf8"));
    const existing = validateNotePassword(stored.password, "local note secret");
    if (existing !== password) {
      throw new Error(`Local note secret vault already exists with a different secret: ${file}`);
    }
    return false;
  }
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.writeFileSync(file, JSON.stringify({
    version: LOCAL_NOTE_SECRET_VERSION,
    createdAt: new Date().toISOString(),
    purpose: "encrypt_local_px402_notes_and_recovery_files",
    password,
  }, null, 2), { mode: 0o600 });
  try {
...
function readOrCreateLocalNotePassword(noteDir) {
  const file = noteSecretVaultPath();
  if (fs.existsSync(file)) {
    const stored = JSON.parse(fs.readFileSync(file, "utf8"));
    if (stored.version !== LOCAL_NOTE_SECRET_VERSION) {
      throw new Error(`Unsupported local note secret version: ${stored.version}`);
    }
    return {
      password: validateNotePassword(stored.password, "local note secret"),
      source: "local_secret_vault",
      path: file,
      created: false,
    };
  }
  if (hasNoteFiles(noteDir)) {
    throw new Error(`PX402 note files exist in ${noteDir}, but no PX402_NOTE_PASSWORD or local secret vault was found. Refusing to create a new password that cannot decrypt existing notes.`);
  }
  const password = crypto.randomBytes(32).toString("base64url");
  writeLocalNoteSecret(file, password, false);
  return {
    password,
    source: "local_secret_vault",
    path: file,
...
function requiredNotePassword(noteDir) {
  const raw = String(process.env.PX402_NOTE_PASSWORD || "").trim();
  if (!raw) {
    return readOrCreateLocalNotePassword(noteDir);
  }
  return {
    password: validateNotePassword(raw, "PX402_NOTE_PASSWORD"),
    source: "env",
    path: null,
    created: false,
  };
...
function config(args = {}) {
  ...
  const noteSecret = requiredNotePassword(noteDir);
  ...
  return {
    ...
    notePassword: noteSecret.password,
    notePasswordSource: noteSecret.source,
    noteSecretPath: noteSecret.path,
    noteSecretCreated: noteSecret.created,
    ...
    cawEvmAddress: process.env.CAW_EVM_ADDRESS || DEFAULT_CAW_EVM_ADDRESS,
...
async function sweepBurnerUsdc(cfg, burner, destination, amount) {
  const validAfter = 0n;
...
async function commandDoctor(cfg) {
  ...
  printJson({
    ...
    noteDir: cfg.noteDir,
    notePasswordSource: cfg.notePasswordSource,
    noteSecretPath: cfg.noteSecretPath,
    noteSecretCreated: cfg.noteSecretCreated,
...
function commandSecretsInit(args) {
  const noteDir = path.resolve(rootDir, args.noteDir || process.env.NOTE_DIR || process.env.PX402_NOTE_DIR || "data/notes");
  if (args.materializeVault) {
    const password = validateNotePassword(process.env.PX402_NOTE_PASSWORD, "PX402_NOTE_PASSWORD");
    const file = noteSecretVaultPath();
    const created = writeLocalNoteSecret(file, password, Boolean(args.force));
    printJson({
      action: "secrets_init",
      mode: "materialize_vault",
      noteDir,
      notePasswordSource: "local_secret_vault",
      noteSecretPath: file,
      noteSecretCreated: created,
      passwordPrinted: false,
    });
    return;
  }
  const secret = requiredNotePassword(noteDir);
  printJson({
    action: "secrets_init",
    noteDir,
    notePasswordSource: secret.source,
    noteSecretPath: secret.path,
    noteSecretCreated: secret.created,
    passwordPrinted: false,
  });
}
...
async function commandRecoverSweep(cfg, args) {
  ...
  const destination = args.toCaw ? cfg.cawEvmAddress : (args.destination || cfg.account.address);
  const client = publicClient(cfg);
  ...
  if (amount === 0n || dryRun) {
    printJson({
      ...
      paymentTx: recovery.paymentTx || null,
      burnerAddress: burner.address,
      destination,
  ...
  const sweep = await sweepBurnerUsdc(cfg, burner, destination, amount);
...
async function commandReturnCaw(cfg, args) {
  const dryRun = !args.execute;
  const warning = returnCawWarning(cfg);
  const latest = await readLatestNote(cfg);
  if (!latest) {
    printJson({ action: "return_caw", dryRun, executable: false, reason: "no_px402_note", destination: cfg.cawEvmAddress, warning });
    return;
  }
  const requested = args.amount !== undefined ? Number(args.amount) : latest.balance;
  if (!Number.isFinite(requested) || requested < 0) throw new Error("--amount must be a non-negative USDC amount.");
  const amountMicro = BigInt(Math.floor(requested * 1_000_000));
  const availableMicro = BigInt(Math.floor(latest.balance * 1_000_000));
  ...
  if (amountMicro === 0n) {
    printJson({
      ...
      sourceNotePath: latest.file,
      noteBalanceUsdc: latest.balance,
      destination: cfg.cawEvmAddress,
      warning,
    });
    return;
  ...
  if (amountMicro > availableMicro) {
    throw new Error(`Return amount ${amountUsdc} exceeds note balance ${latest.balance}.`);
  }
  if (dryRun) {
    printJson({
      action: "return_caw",
      ...
      sourceNotePath: latest.file,
      noteBalanceUsdc: latest.balance,
      returnAmountUsdc: amountUsdc,
      destination: cfg.cawEvmAddress,
      plan: "px402 note -> private withdrawal burner -> EIP-3009 USDC sweep to CAW",
      alternatives: ["spend-down", "hold-note"],
      warning,
    });
    return;
  }
  if (!args.ackReturnLink) {
    throw new Error("Refusing return:caw --execute without explicit acknowledgement. This action can create a public burner-to-CAW link. Re-run with --ack-return-link only after the user accepts this tradeoff.");
  }

  assertBaseOnlyConfig();
  assertRemoteCircuitsAccepted(cfg);
  await timedStage(cfg, "ensure_dust_eth", () => ensureDustEth(cfg, false), { provider: "return_caw" });

  const { PrivacySDK, getNoteBalance } = privacy();
  const sdk = new PrivacySDK({ chain: "base", rpcUrl: cfg.rpcUrl });
  sdk.setNote(latest.note);
  const paymentResult = await timedStage(cfg, "private_note_return_withdrawal", () => sdk.makePayment(latest.note, "", amountUsdc), {
    provider: "return_caw",
    sourceNotePath: relativeOrValue(latest.file),
    noteBalanceBeforeUsdc: latest.balance,
    returnAmountUsdc: amountUsdc,
  ...
  const burner = accounts().privateKeyToAccount(paymentResult.burnerPrivateKey);
  appendRunLog(cfg, {
    event: "return_private_withdrawal_made",
    provider: "return_caw",
    burnerAddress: burner.address,
    paymentTx: paymentResult.txHash,
    amountUsdc,
  });
  const recoveryPath = await timedStage(cfg, "write_return_recovery_secret", async () => encryptLocalSecret(cfg, {
    provider: "return_caw",
    createdAt: new Date().toISOString(),
    burnerPrivateKey: paymentResult.burnerPrivateKey,
    burnerAddress: burner.address,
    paymentTx: paymentResult.txHash,
    intendedPayTo: cfg.cawEvmAddress,
    intendedAmountMicro: amountMicro.toString(),
  }), { provider: "return_caw", payer: burner.address });
  const updatedNote = sdk.getUpdatedNote();
  const updatedNotePath = updatedNote
    ? await timedStage(cfg, "write_return_updated_note", () => writeEncryptedNote(cfg, updatedNote, {
        source: "return_caw",
        previousNotePath: latest.file,
        destination: cfg.cawEvmAddress,
        returnAmountUsdc: amountUsdc,
      }), { provider: "return_caw" })
    : null;
  ...
  const client = publicClient(cfg);
  const burnerBalance = await timedStage(cfg, "read_return_burner_balance", () => client.readContract({
    address: BASE_USDC,
    abi: viem().erc20Abi,
    functionName: "balanceOf",
    args: [burner.address],
  }), { provider: "return_caw", burnerAddress: burner.address });
  const sweepAmount = burnerBalance < amountMicro ? burnerBalance : amountMicro;
  if (sweepAmount === 0n) {
    throw new Error(`Return burner has zero USDC after private withdrawal: ${burner.address}; recoveryPath=${recoveryPath}`);
  }
  const sweep = await timedStage(cfg, "sweep_return_burner_to_caw", () => sweepBurnerUsdc(cfg, burner, cfg.cawEvmAddress, sweepAmount), {
    provider: "return_caw",
    burnerAddress: burner.address,
    destination: cfg.cawEvmAddress,
    amountUsdc: viem().formatUnits(sweepAmount, 6),
  });
  const receiptPath = writeReceipt(cfg, {
    provider: "return_caw",
    createdAt: new Date().toISOString(),
    ...
    noteBalanceBeforeUsdc: latest.balance,
    noteBalanceAfterUsdc: updatedNote ? getNoteBalance(updatedNote) : null,
    privateWithdrawalTx: paymentResult.txHash,
    burnerAddress: burner.address,
    destination: cfg.cawEvmAddress,
    amountUsdc: viem().formatUnits(sweepAmount, 6),
    transactionHash: sweep.hash,
...
function usage() {
  console.log(`Usage:
  npm run privacy -- secrets:init [--note-dir data/notes]
  ...
  npm run privacy -- recover:sweep --file data/recovery/recovery-....json [--to-caw|--destination 0x...] [--execute]
  npm run privacy -- return:caw [--amount 0.01] [--execute --ack-return-link]
...
async function main() {
  ...
  if (command === "secrets:init") return commandSecretsInit(args);
...
main().catch((error) => {
  try {
    ...
    const logDir = path.resolve(rootDir, process.env.RUN_LOG_DIR || "data/run-logs");
    appendRunLog({ runId, runLogPath: path.join(logDir, `${safeFileSegment(runId)}.jsonl`) }, {
      event: "command_error",
      command,
      error: { name: error.name, message: error.message, stack: error.stack },
    });
